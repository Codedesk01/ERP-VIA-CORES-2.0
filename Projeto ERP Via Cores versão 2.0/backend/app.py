...
app.config["SQLALCHEMY_DATABASE_URI"] = "sqlite:///erp_viacores.db"
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                try:
                    ...
                    uid = ident.get("id") if ident else None
                except Exception:
                    uid = None
                log = SystemLog(
                    actor_id=uid,
...
def logout():
    # JWT stateless (frontend só descarta o token)
...
def current_user_id():
    try:
        ident = get_jwt_identity()
        return ident.get("id") if ident else None
    except Exception:
        return None
