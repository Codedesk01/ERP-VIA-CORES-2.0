...
function logAction(action) {
    ...
    logs.unshift(`${timestamp}: ${action} (Usuário: ${currentUser ? currentUser.username : 'N/A'})`);
    if (logs.length > 100000) logs.pop();
    saveData();
...
function login() {
    ...
    if (user) {
        ...
        logAction(`Tentativa de login falhou para o usuário: ${username}`);
...
/**
 * Carrega os itens de navegação na sidebar, agora com lógica para o modo retrátil.
 * Adiciona a classe 'sidebar-text' aos nomes dos módulos e um atributo 'title'
 * para funcionar como tooltip quando o menu estiver recolhido.
 */
function loadMenu() {
    const nav = document.getElementById('nav-menu');
    nav.innerHTML = ''; // Limpa o menu antes de recarregar
    ...
    const menuItems = [
        ...
        { id: 'chat', label: 'Chat Interno', icon: 'fa-comments', module: 'chat', action: 'visualizar' }, // <-- ADICIONE ESTA LINHA
    ...
    menuItems.forEach(item => {
        ...
        if ((isAdminItem && userIsAdmin) || (!isAdminItem && hasPermission(item.module, item.action))) {
            // **AQUI ESTÁ A MÁGICA**
            // O texto (label) agora está dentro de um <span> com a classe 'sidebar-text'.
            // O botão inteiro recebe um 'title' com o nome do item.
            nav.innerHTML += `
                <button 
                    ...
                    title="${item.label}">  <!-- ATRIBUTO TITLE PARA O TOOLTIP -->
                    
                    <i class="fas ${item.icon} w-6 text-center text-lg"></i>
                    <span class="sidebar-text whitespace-nowrap">${item.label}</span> <!-- TEXTO COM A CLASSE PARA RECOLHER -->
                </button>`;
        }
    });

    // Adiciona os botões de "Alterar Senha" e "Logout" que não são afetados pelo recolhimento
...
function loadDynamicData(sectionId) {
    ...
    'admin-dashboard': () => {
        // Agora, ao carregar o dashboard, ele executa ambas as funções.
        loadAdminDashboard();      // Carrega as métricas, gráficos e logs.
        setupNotificationSender(); // Prepara o formulário de envio de notificações.
    },
    // --- FIM DA ALTERAÇÃO ---
    ...
    'costura': loadCostura,
    'chat': loadChat, // <-- ADICIONE ESTA LINHA
...
function createUser() {
    ...
    logAction(`Usuário criado: ${username} com o papel de ${role}`);
...
function loadPermissionModules() {
    const modules = [
        ...
        { key: 'chat', label: 'Chat Interno', actions: ['visualizar', 'enviar'] } // <-- ADICIONE ESTA LINHA
    ];
...
function deleteItem(id) {
    ...
    if (confirm(`Tem certeza que deseja excluir o item ${item.sku} da prateleira ${item.prateleira}?`)) {
        ...
        saveData();
        applyFilters();
        logAction(`Item de estoque excluído: SKU ${item.sku}, Prateleira ${item.prateleira}`);
...
/**
 * Processa o texto de pedidos da Shopee. VERSÃO 5.0 - FINAL E ROBUSTA.
 * Esta versão foi reescrita para garantir máxima compatibilidade e corrigir regressões.
 *
 * @param {string} text - O conteúdo copiado da página da Shopee.
 * @returns {object} Um objeto com { pedidosValidos, pedidosCancelados, pedidosComErro }.
 */
function parseShopeeTexto(text) {
    // ... (código da versão 5.0 que você confirmou que funciona)
    // Esta função já identifica corretamente que um item tem "quantidade: 2", por exemplo.
    ...
    blocos.forEach((bloco, index) => {
        ...
        while ((match = regexItem.exec(bloco)) !== null) {
            ...
            const skuFinal = partesSku.length > 1 ? partesSku[1] : partesSku[0];
        ...
        itensEncontrados.forEach(item => {
            // A função agora cria um único objeto com a quantidade total
            const pedidoData = {
                id: idPedido,
                marketplace: 'Shopee',
                dataColeta,
                tipoEntrega,
                sku: item.sku,
                quantidade: item.quantidade, // A quantidade total é passada aqui
    ...
    if (selecionados.length === 0) {
        ...
        selecionados.forEach(checkbox => {
            ...
            if (itemIndex !== -1) {
                ...
                costura.push({
                    lote: `LOTE-${Date.now() + itensMovidos}`,
                    op: itemMovido.op,
                    sku: itemMovido.sku,
                    ...
                    dataColeta: itemMovido.dataColeta // <-- ADICIONE ESTA LINHA
                });
                itensMovidos++;
            }
        });
        ...
        if (itensMovidos > 0) {
            ...
            loadCostura(); // Atualiza a tela de costura também
            logAction(`${itensMovidos} item(ns) movidos da Produção para a Costura.`);
            showToast(`${itensMovidos} item(ns) enviados para a Costura.`, 'success');
        }
    }
}
...
function processarPedidosManuais() {
    ...
    if (pedidosAdicionados > 0) {
        ...
        logAction(`${pedidosAdicionados} itens do pedido manual '${idFinal}' foram cadastrados.`);
...
function processarItemUnico(item, forcarProducao) {
    ...
    if (forcarProducao) {
        ...
        if (!temEstoque) {
            showToast(`Ação inválida. O item ${item.sku} não tem estoque.`, 'error');
            return;
        }
...
function renderCardsCostura(item) {
    ...
    if (isFinalizado) {
        if (isAdmin || isDoUsuario) {
            ...
            buttonHtml = `<button class="w-full bg-gray-400 text-white px-4 py-2 rounded-lg cursor-not-allowed" disabled>
                            <i class="fas fa-check-double mr-2"></i>Finalizado por ${item.usuario}
                          </button>`;
        }
    } else if ((isEmAndamento || isPausado) && isDoUsuario) {
    ...
    } else if (!isEmAndamento && !isPausado) {
        buttonHtml = `<button onclick="iniciarTarefaCostura('${item.lote}')" class="w-full bg-blue-600 text-white px-4 py-2 rounded-lg font-semibold hover:bg-blue-700"><i class="fas fa-play mr-2"></i>Abrir Tarefa</button>`;
    } else {
        buttonHtml = `<button class="w-full bg-gray-400 text-white px-4 py-2 rounded-lg cursor-not-allowed" disabled><i class="fas fa-lock mr-2"></i>Em uso por ${item.usuario}</button>`;
    }
...
function forcarEnvioParaExpedicao(loteId) {
    ...
    if (confirm(`Tem certeza que deseja forçar o envio do lote ${loteId} para a expedição?`)) {
        ...
        logAction(`Envio do lote ${loteId} forçado por ${currentUser.username}.`);
    }
}

// =================================================================================
// LÓGICA DO CONTROLE DE TAREFA E CRONÔMETRO (VERSÃO FINAL REVISADA)
// =================================================================================
...
function iniciarTarefaCostura(loteId) {
    ...
    if (item.usuario && item.usuario !== currentUser.username) {
        return showToast(`Este lote já está sendo trabalhado por ${item.usuario}.`, 'error');
    }
    ...
    tarefaCosturaAtiva = {
        ...
        sku: item.sku,
        marketplace: item.marketplace,
        inicio: item.inicioTimestamp || null,
        tempoPausado: item.tempoPausadoAcumulado || 0,
        ultimoPause: null
    };
    ...
    document.getElementById('btn-enviar-expedicao').classList.add('hidden');
    document.getElementById('btn-retomar-costura').classList.add('hidden');

    if (item.status === 'Em Andamento' || item.status === 'Pausado') {
        document.getElementById('btn-iniciar-trabalho').classList.add('hidden');
        document.getElementById('botoes-pausa-retomada').classList.remove('hidden');
        document.getElementById('btn-finalizar-costura').classList.remove('hidden');
        
        if(item.status === 'Pausado') {
            document.getElementById('btn-pausar-costura').classList.add('hidden');
            document.getElementById('btn-retomar-costura').classList.remove('hidden');
            document.getElementById('cronometro-status').innerText = 'Pausado';
        } else {
            document.getElementById('btn-pausar-costura').classList.remove('hidden');
            document.getElementById('cronometro-status').innerText = 'Tempo decorrido';
        }
        iniciarCronometro();
    } else {
        document.getElementById('cronometro-costura').innerText = '00:00:00';
        document.getElementById('cronometro-status').innerText = 'Pronto para Iniciar';
        document.getElementById('btn-iniciar-trabalho').classList.remove('hidden');
        document.getElementById('botoes-pausa-retomada').classList.add('hidden');
...
function iniciarCronometroETrabalho() {
    ...
    if (!tarefaCosturaAtiva.inicio) {
        tarefaCosturaAtiva.inicio = Date.now();
    }
    
    const item = costura.find(c => c.lote === tarefaCosturaAtiva.lote);
    if (item) {
        item.status = 'Em Andamento';
        item.usuario = currentUser.username;
        item.inicioTimestamp = tarefaCosturaAtiva.inicio;
        saveData();
    }
...
function iniciarCronometro() {
    clearInterval(cronometroCosturaInterval);
    const cronometroEl = document.getElementById('cronometro-costura');
    if (!tarefaCosturaAtiva || !tarefaCosturaAtiva.inicio) {
        cronometroEl.innerText = '00:00:00';
        return;
    }

    cronometroCosturaInterval = setInterval(() => {
        if (tarefaCosturaAtiva.ultimoPause) {
            return;
        }
        const agora = Date.now();
        const decorrido = (agora - tarefaCosturaAtiva.inicio) - tarefaCosturaAtiva.tempoPausado;
        
        const segundos = Math.floor((decorrido / 1000) % 60).toString().padStart(2, '0');
        const minutos = Math.floor((decorrido / (1000 * 60)) % 60).toString().padStart(2, '0');
        const horas = Math.floor(decorrido / (1000 * 60 * 60)).toString().padStart(2, '0');
        cronometroEl.innerText = `${horas}:${minutos}:${segundos}`;
    }, 1000);
}

function pausarTarefaCostura() {
    if (!tarefaCosturaAtiva) return;
    
    tarefaCosturaAtiva.ultimoPause = Date.now();
    clearInterval(cronometroCosturaInterval);
    cronometroCosturaInterval = null;
...
function retomarTarefaCostura() {
    if (!tarefaCosturaAtiva || !tarefaCosturaAtiva.ultimoPause) return;

    const tempoPausaAtual = Date.now() - tarefaCosturaAtiva.ultimoPause;
    tarefaCosturaAtiva.tempoPausado += tempoPausaAtual;
    tarefaCosturaAtiva.ultimoPause = null;

    const item = costura.find(c => c.lote === tarefaCosturaAtiva.lote);
    if (item) {
        item.status = 'Em Andamento';
        item.tempoPausadoAcumulado = tarefaCosturaAtiva.tempoPausado;
        saveData();
    }
...
function finalizarTarefaCostura() {
    if (!tarefaCosturaAtiva) return;

    clearInterval(cronometroCosturaInterval);
    cronometroCosturaInterval = null;

    const tempoTotalFormatado = document.getElementById('cronometro-costura').innerText;
    const item = costura.find(c => c.lote === tarefaCosturaAtiva.lote);
    
    if (item) {
        item.status = 'Finalizado';
        item.tempoCostura = tempoTotalFormatado;
        saveData();
    }
    ...
    document.getElementById('cronometro-status').innerText = `Finalizado em ${tempoTotalFormatado}`;

    showToast(`Item finalizado.`, 'info');
    logAction(`Item ${tarefaCosturaAtiva.lote} finalizado por ${currentUser.username} em ${tempoTotalFormatado}.`);
    loadCostura();
}

function pausarEFecharModalCostura() {
    if (tarefaCosturaAtiva) {
        if (tarefaCosturaAtiva.inicio && !tarefaCosturaAtiva.ultimoPause) {
            pausarTarefaCostura();
        } else if (!tarefaCosturaAtiva.inicio) {
            const item = costura.find(c => c.lote === tarefaCosturaAtiva.lote);
            if (item) {
                item.status = 'Aguardando Costura';
                delete item.usuario;
                saveData();
                loadCostura();
            }
 ...
 * NOVA LÓGICA: Renderiza um card diferente e não clicável para pacotes da Shopee,
 * que já chegam "conferidos" da costura.
 * @param {object} pacote - O objeto do pacote a ser renderizado.
 * @returns {string} O HTML do card.
 */
function renderCardPacotePronto(pacote) {
    const { id, cliente, itensPresentes, tipoEntrega, marketplace } = pacote;
    ...
    // *** INÍCIO DA LÓGICA VISUAL PARA SHOPEE ***
    const isShopeeAutoChecked = marketplace === 'Shopee';

    // Define classes e comportamentos com base no marketplace
    const cardCursor = isShopeeAutoChecked ? 'cursor-default' : 'cursor-pointer';
    const onClickAction = isShopeeAutoChecked ? '' : `onclick="abrirModalConferencia('${id}')"`;
    const overlayColor = isShopeeAutoChecked ? 'from-black/95 via-black/70' : 'from-black/90 via-black/60';
    const hoverEffect = isShopeeAutoChecked ? '' : 'group-hover:scale-110';

    // Indicador visual para o card da Shopee
    const shopeeIndicator = isShopeeAutoChecked
        ? `<div class="absolute top-3 left-3 bg-orange-500 text-white text-xs font-bold px-3 py-1 rounded-full flex items-center gap-2">
    ...
    const motoboyIndicator = isMotoboy
        ? `<span class="absolute top-3 right-3 bg-purple-600 text-white text-xs font-bold px-2 py-1 rounded-full flex items-center gap-1 animate-pulse"><i class="fas fa-motorcycle"></i> MOTOBOY</span>`
        : '';
    ...
    return `
        <div ${onClickAction}
            ...
            <div class="absolute inset-0 bg-no-repeat bg-cover bg-center transition-transform duration-300 ${hoverEffect}"
                 style="background-image: url('${imageUrl}');"></div>

            <div class="absolute inset-0 bg-gradient-to-t ${overlayColor} to-transparent"></div>
...
/**
 * Função ÚNICA: Imprime a etiqueta e, em seguida, dá baixa no pacote.
 */
function imprimirEtiquetaEDarBaixa(pedidoId) {
    const itemExpedicao = expedicao.find(item => item.pedidoId === pedidoId && item.zplContent);
    if (!itemExpedicao || !itemExpedicao.zplContent) {
        showToast('Etiqueta ZPL não encontrada para este pedido. Associe a etiqueta primeiro.', 'error');
        return;
    }

    // 1. Gera e baixa o arquivo .txt
    const blob = new Blob([itemExpedicao.zplContent], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = `etiqueta_${pedidoId.replace('#', '')}.txt`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
    showToast(`Etiqueta para o pedido ${pedidoId} gerada.`, 'success');

    // 2. Dá baixa no pacote
    ...
    if (itensEnviadosCount > 0) {
        saveData();
        logAction(`Pacote ${pedidoId} (${itensEnviadosCount} itens) teve baixa automática após impressão.`);
        showToast(`Baixa automática do pacote ${pedidoId} realizada!`, 'success');
        
        // 3. Atualiza a UI sem fechar o modal
        loadExpedicao(); // Recarrega a lista de pacotes no fundo
        
        // Desabilita o botão para evitar cliques duplos e muda o texto
        const btn = document.getElementById('btn-imprimir-dar-baixa');
        if (btn) {
            btn.disabled = true;
            btn.innerHTML = `<i class="fas fa-check-circle"></i> Baixa Realizada com Sucesso`;
        ...
        try {
            ...
            etiquetasIndividuais.forEach(etiquetaZpl => {
                ...
                if (encontrouItem) {
                    ...
                    logAction(`Etiqueta associada: Venda ${idVenda} -> Rastreio ${codigoRastreio}.`);
                } else {
        ...
        try {
            ...
            for (let etiquetaZPL of etiquetasIndividuais) {
                ...
                if (itemExpedicao) {
                    ...
                    etiquetasAssociadas++;
                    logAction(`Etiqueta Shopee associada: Pedido ${idPedidoShopee} -> Rastreio ${codigoRastreio}.`);
                } else {
